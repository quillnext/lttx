...
//     if (currentStep === 0) {
//       ['username', 'fullName', 'email', 'phone', 'location', 'languages', 'responseTime', 'pricing'].forEach(field => {
//         if (!formData[field]?.trim()) newErrors[field] = 'This field is required';
//       });
//       if (!formData.dateOfBirth) newErrors.dateOfBirth = 'Date of birth is required';
//       if (formData.email && !validateEmail(formData.email)) newErrors.email = 'Invalid email address';
...
//                 type="button"
//                 onClick={handleBack}
//                 className="px-6 py-2 text-sm font-semibold text-[var(--primary)] hover:text-gray-800 rounded-xl"
//               >
//                 Back
//               </button>
//             )}
...
//                 type="button"
//                 onClick={handleNext}
//                 className="px-6 py-2 text-sm font-semibold text-white bg-[var(--primary)] rounded-xl hover:bg-green-700"
//               >
//                 Next
//               </button>
//             ) : (
...
//                 onClick={handleSubmit}
//                 disabled={isSubmitting}
//                 className={`px-6 py-2 text-sm font-semibold text-white rounded-xl ${isSubmitting ? 'bg-green-400 cursor-not-allowed' : 'bg-[var(--primary)] hover:bg-green-700'}`}
//               >
//                 {isSubmitting ? 'Submitting...' : profileId ? 'Update Profile' : 'Submit'}
//               </button>
//             )}
...
export default function CompleteProfile() {
  ...
  const [referralCodeStatus, setReferralCodeStatus] = useState('');
  const [referrerUsername, setReferrerUsername] = useState('');
  const [customExpertise, setCustomExpertise] = useState(''); // State for custom expertise input
        ...
        try {
          ...
          if (profileSnap.exists()) {
            ...
            setFormData({
              ...
              services: data.services?.length ? data.services : [''],
              regions: data.regions?.length ? data.regions : [],
              expertise: Array.isArray(data.expertise) ? data.expertise : [], // Initialize expertise
              experience: data.experience?.length
                ? data.experience.map(exp => ({
                    title: exp.title || '',
                    company: exp.company || '',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const profileData = {
        ...
        services: formData.services,
        regions: formData.regions,
        expertise: formData.expertise, // Include expertise in profile data
        experience: formData.experience.map(exp => ({
          title: exp.title,
          company: exp.company,
          startDate: exp.startDate ? formatDate(exp.startDate, 'YYYY-MM') : '',
