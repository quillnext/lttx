...
//   // Render QuestionSearchPage if search query parameter exists
//   if (searchTerm) {
//     return <QuestionSearchPage />;
//   }

//   // Original Home page logic
//   const [formData, setFormData] = useState({
//     searchQuery: "",
//   });
//   const [loading, setLoading] = useState(false);
...
//                 disabled={loading}
//                 className={`bg-[var(--secondary)] text-[var(--primary)] font-semibold px-6 py-3 rounded-full shadow hover:scale-105 transition ${loading ? "opacity-50 cursor-not-allowed" : ""}`}
//               >
//                 Ask Expert
//               </button>
//             </div>
...
export default function Home() {
  ...
  const handleSearchRedirect = async () => {
    ...
    try {
      const keywords = extractKeywords(formData.searchQuery);
      if (keywords.length === 0) {
        const slug = toSlug(formData.searchQuery);
