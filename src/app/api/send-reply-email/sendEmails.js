...
.container { max-width: 600px; margin: auto; background: #fff; border-radius: 16px; border: 1px solid #e2e2e2; overflow: hidden; box-shadow: 0 8px 24px rgba(0,0,0,0.08); }
.content { padding: 32px; }
.footer { font-size: 13px; color: #888; text-align: center; margin-top: 40px; }
h2 { color: #36013F; margin-bottom: 16px; }
p { font-size: 15px; line-height: 1.6; }
a { color: #36013F; }
.question-box, .reply-box { background-color: #fff; padding: 16px; border-radius: 8px; margin-bottom: 16px; border: 1px solid #ddd; }
</style></head><body>
<div class="container">
...
<div class="question-box"><p><strong>Your Question:</strong><br/>"${question}"</p></div>
<div class="reply-box"><p><strong>Expert’s Response:</strong><br/>"${reply}"</p></div>
...
<p class="footer">© ${year} XMyTravel • <a href="https://xmytravel.com">xmytravel.com</a><br/>For support: <a href="mailto:info@xmytravel.com">info@xmytravel.com</a></p>
</div></div></body></html>`;

export async function sendReplyEmails({ userEmail, userName, expertName, question, reply }) {
  const year = new Date().getFullYear();

  const userHtml = emailTemplate({ userName, expertName, question, reply, year, type: "user" });
  const adminHtml = emailTemplate({ userName, expertName, question, reply, year, type: "admin" });
