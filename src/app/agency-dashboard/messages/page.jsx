...
export default function Messages() {
          ...
          .on(
            ...
            (payload) => {
              ...
              if (eventType === 'INSERT') {
                ...
                if (newRow.expert_id === profileIdRef.current) {
                  const item = newRow;
                  const newLead = {
                    ...item,
  ...
  const fetchExpertProfile = async (user) => {
    try {
      const emailParam = user.email ? `&email=${encodeURIComponent(user.email)}` : "";
      const response = await fetch(`/api/profile/by-uid?uid=${encodeURIComponent(user.uid)}${emailParam}`);
      const result = await response.json();
      if (result.profile) {
        const mapped = mapSupabaseProfile(result.profile);
