  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-md">
      <p className="text-gray-600 mb-4">
        Set your default weekly hours. Select time slots, then select the days you want to apply them to.
      </p>
          ...
          {allTimeSlots.map((slot) => (
            <button
              ...
              className={`p-2 rounded-md text-sm transition-colors ${
                selectedSlots.includes(slot) ? "bg-[#36013F] text-white" : "bg-white border hover:bg-gray-100"
              }`}
            >
              {slot}
            </button>
      ...
      <div className="mt-6 flex justify-between items-center gap-4">
        ...
        <button
          ...
          disabled={saving}
          className="flex items-center gap-2 bg-[#36013F] text-white py-2 px-6 rounded-lg hover:bg-[#4a0152] transition-colors disabled:opacity-50"
        >
          {saving ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}
          {saving ? "Saving..." : "Apply to Selected Days"}
        </button>
...
export default function AvailabilityPage() {
  const [user, setUser] = useState(null);
  const [supabaseProfileId, setSupabaseProfileId] = useState(null);
  const [selectedDate, setSelectedDate] = useState(new Date());
  const [dailySchedule, setDailySchedule] = useState([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  ...
  // Temporary function to initialize default schedule for new users
  const initializeDefaultSchedule = async (userId) => {
    const recurringRef = doc(db, "ExpertRecurringAvailability", userId);
    try {
      ...
      if (!docSnap.exists()) {
        ...
        console.log("Default schedule initialized for user:", userId);
        setWeeklySchedule(defaultSchedule); // Update state to reflect default schedule
      } else {
        setWeeklySchedule(docSnap.data().schedule || {});
      }
    } catch (error) {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged((currentUser) => {
      if (currentUser) {
        setUser(currentUser);
        // Initialize profile and schedule
        const profileRef = doc(db, "Profiles", currentUser.uid);
        getDoc(profileRef).then(async (docSnap) => {
          if (docSnap.exists()) {
            ...
            try {
              ...
              const data = await getProfileByUidOrEmail(supabase, currentUser.uid, currentUser.email);
              if (data) {
        ...
        // Initialize or fetch recurring schedule
        initializeDefaultSchedule(currentUser.uid);
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleSaveAvailability = async () => {
    ...
    const slotsToSave = dailySchedule
  ...
  const handleOnlineToggle = async () => {
    ...
    try {
      ...
      console.error("Failed to update online status in Firestore:", error);
    }

    // 2. Try Supabase
    try {
      let profileId = supabaseProfileId;
      if (!profileId) {
        const { supabase } = await import("@/lib/supabase");
        const { getProfileByUidOrEmail } = await import("@/lib/supabaseProfile");
        const data = await getProfileByUidOrEmail(supabase, user.uid, user.email);
        profileId = data?.id || user.uid;
        setSupabaseProfileId(profileId);
      }
      const response = await fetch("/api/profile/by-uid", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ profileId, payload: { is_online: newStatus } }),
      });
      if (!response.ok) {
        const errData = await response.json();
        throw new Error(errData.error || "Failed to update online status via API");
      }
    } catch (error) {
      console.error("Failed to update online status in Supabase:", error);
      setIsOnline(!newStatus); // Revert on error
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-gray-200 p-1 rounded-xl flex gap-1 w-full sm:w-auto mb-6 max-w-sm">
            <button
              ...
              className={`px-6 py-2 rounded-lg font-semibold transition-colors w-full ${
                activeTab === "custom" ? "bg-white shadow text-[#36013F]" : "text-gray-600 hover:bg-gray-300"
              }`}
            >
              Custom Dates
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-lg font-semibold transition-colors w-full ${
                activeTab === "recurring" ? "bg-white shadow text-[#36013F]" : "text-gray-600 hover:bg-gray-300"
              }`}
            >
              Weekly Hours
            </button>
              ...
              <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-md">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-[#36013F] text-white py-2 px-4 rounded-lg hover:bg-[#4a0152] transition-colors disabled:opacity-50"
                  >
                    {saving ? <Loader2 className="animate-spin" size={20} /> : <Save size={20} />}
                    {saving ? "Saving..." : "Save Changes"}
                  </button>
                ...
                {loading ? (
                  <div className="flex items-center justify-center h-64">
                    <Loader2 className="animate-spin h-10 w-10 text-[#36013F]" />
                  </div>
                ) : (
                  <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 max-h-[28rem] overflow-y-auto pr-2">
                    {dailySchedule.map(({ time, status }) => (
