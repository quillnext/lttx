...
export default async function ProfilePage({ params }) {
  ...
  const serializeTimestamp = (timestamp) => {
    ...
    if (timestamp instanceof Object && "seconds" in timestamp && "nanoseconds" in timestamp) {
  ...
  const serializedProfile = {
    ...profile,
    timestamp: serializeTimestamp(profile.timestamp),
    approvalTimestamp: serializeTimestamp(profile.approvalTimestamp),
    dateOfBirth: serializeTimestamp(profile.dateOfBirth),
    // Ensure photo is a string (URL), not a storage reference
    photo: typeof profile.photo === "string" ? profile.photo : "/default.jpg",
    // Serialize experience dates
    experience: profile.experience?.map((exp) => ({
      ...exp,
      startDate: typeof exp.startDate === "string" ? exp.startDate : serializeTimestamp(exp.startDate),
      endDate: typeof exp.endDate === "string" ? exp.endDate : serializeTimestamp(exp.endDate),
    })),
    // Ensure other fields are plain data
  ...
  const parseDate = (dateString) => {
    if (!dateString || typeof dateString !== "string") return null;
    if (dateString === "Present") return null; // Handle "Present" case
  ...
  const sortedExperience = serializedProfile.experience
    ?.map((exp) => ({
      ...exp,
      startDateParsed: parseDate(exp.startDate),
      startDateFormatted: formatDate(exp.startDate),
    ...
    .filter((exp) => exp.startDateParsed)
    .sort((a, b) => b.startDateParsed - a.startDateParsed)
    .map(({ startDateParsed, ...exp }) => exp);
