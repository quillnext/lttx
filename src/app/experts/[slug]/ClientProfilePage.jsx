...
import { Share2, MessageCircle, Calendar as CalendarIcon, Loader2, PowerOff } from "lucide-react"; 
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await runTransaction(db, async (transaction) => {
                // We need to check both custom and recurring availability for the source of the slot
                ...
                // If it's a recurring slot, it might not have a document in ExpertAvailability yet,
                // so we just proceed to book it. The fetch logic will filter it out next time.
            ...
            <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md text-center">
                    ...
                    <p className="text-gray-700">
                        ...
                        <strong>{selectedDate.toLocaleDateString()} at {selectedSlot.startTime}</strong> is confirmed.
                    </p>
                    <p className="text-gray-600 mt-2">Check your email for details.</p>
                    <button onClick={onClose} className="mt-6 w-full bg-[#36013F] text-white py-3 rounded-xl hover:bg-[#4a0152]">
                        Close
                    </button>
                </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-lg relative">
                 ...
                 <div className="bg-gray-50 p-4 rounded-lg mb-4 border border-gray-200">
                    <p><strong>Expert:</strong> {expert.fullName}</p>
                 ...
                 <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        ...
                        <input type="text" id="name" value={name} onChange={(e) => setName(e.target.value)} required className="mt-1 w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#36013F] focus:border-transparent" disabled={!!user && !!user.displayName} />
                    </div>
                     <div>
                        <label htmlFor="email" className="block text-sm font-medium text-gray-700">Email Address</label>
                        <input type="email" id="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="mt-1 w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-[#36013F] focus:border-transparent" disabled={!!user} />
                    </div>
                    <div>
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-[#36013F] text-white py-3 rounded-xl hover:bg-[#4a0152] disabled:opacity-50 transition-colors">
...
const BookingComponent = ({ expert, weeklySchedule }) => {
  ...
  const fetchSlotsForDate = useCallback(async (date) => {
    ...
    try {
        // 1. Check for custom availability first
        const availabilityRef = doc(db, "ExpertAvailability", `${expert.id}_${dateStr}`);
        const availabilitySnap = await getDoc(availabilityRef);
        
        let potentialSlots = [];
        if (availabilitySnap.exists()) {
            // Custom schedule found for this date, use it
            potentialSlots = availabilitySnap.data().slots?.filter(s => s.status === 'available').map(s => s.startTime) || [];
        } else {
            // No custom schedule, fall back to weekly recurring schedule
            potentialSlots = weeklySchedule[dayOfWeek] || [];
        }

        // 2. Fetch bookings for this date to filter out booked slots
        ...
        // 3. Filter out booked times from potential slots and format for UI
  ...
  const CustomCalendarInput = ({ value, onClick }) => (
    <button onClick={onClick} className="flex items-center gap-2 w-full justify-center px-4 py-2 rounded-lg border-2 border-gray-300 bg-white hover:bg-gray-50 transition-colors">
  ...
  return (
    <>
        ...
        <div className="px-5 pb-5 text-sm text-gray-700 leading-relaxed">
            {!expert.isOnline ? (
                <div className="text-center py-10 px-4 border-2 border-dashed rounded-2xl mt-6 bg-gray-50">
                    <div className="flex justify-center text-gray-400 mb-4"><PowerOff size={48} /></div>
            ...
            <div>
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                     <button onClick={setToday} className="px-4 py-2 rounded-lg border-2 bg-white hover:bg-gray-50 border-gray-300 transition-colors">Today</button>
                     <button onClick={setTomorrow} className="px-4 py-2 rounded-lg border-2 bg-white hover:bg-gray-50 border-gray-300 transition-colors">Tomorrow</button>
                     <DatePicker
                ...
                {loadingSlots && (
                    <div className="flex items-center justify-center h-48"><Loader2 className="animate-spin h-8 w-8 text-[#36013F]"/></div>
                )}

                {selectedDate && !loadingSlots && (
                    <div className="mt-4">
                        <h3 className="font-semibold text-gray-800 mb-2">2. Pick a Time for {selectedDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</h3>
                        {slotsForSelectedDate.length > 0 ? (
                             <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                                {allTimeSlots.map(time => {
                                    ...
                                    return (
                                        <button key={time} onClick={() => isAvailable && handleSlotClick(slot)} disabled={!isAvailable}
                                            className={`p-2 border rounded-lg text-center transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#36013F] ${ isAvailable ? 'bg-gray-100 hover:bg-[#36013F] hover:text-white' : 'bg-gray-50 text-gray-400 cursor-not-allowed line-through' }`}>
                                            {time}
                                        </button>
                                    );
                                })}
                           ...
                           <p className="text-gray-500 text-center py-10">This expert has no availability on this date. Please try another day.</p>
                        )}
                    </div>
                )}
            </div>
      ...
      {isBookingModalOpen && selectedDate && selectedSlot && (
        <BookingConfirmationModal expert={expert} selectedDate={selectedDate} selectedSlot={selectedSlot} onClose={() => setIsBookingModalOpen(false)} onBookingSuccess={handleBookingSuccess} />
      )}
    </>
  );
}
      ...
      <div className="text-gray-800 relative mt-20 pb-16 lg:pb-0">
        <div className="max-w-7xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
          <aside className="lg:col-span-1 space-y-4">
            ...
            <button
              onClick={handleShare}
              className="fixed bottom-4 left-4 bg-secondary text-secondary-foreground p-3 rounded-full shadow-lg hover:bg-opacity-90 transition-all flex items-center gap-2 z-50 hidden lg:flex"
              title="Share Profile"
            >
              <Share2 />
            ...
            <button
              onClick={() => setIsModalOpen(true)}
              className="fixed bottom-4 right-4 bg-primary text-primary-foreground p-3 rounded-full shadow-lg hover:bg-opacity-90 transition-all flex items-center gap-2 z-50 hidden lg:flex"
              title="Ask a Question"
            >
              <MessageCircle />
          ...
          <section className="lg:col-span-2 space-y-4">
            ...
            {/* Already Answered Questions Section */}
            <details
              className="group bg-white rounded-2xl shadow border border-[#F4D35E30] overflow-hidden"
            >
              <summary className="flex items-center justify-between px-5 py-4 cursor-pointer select-none transition-colors duration-200 hover:bg-gray-50">
