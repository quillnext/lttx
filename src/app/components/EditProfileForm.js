...
// Dynamically import components with robust error handling
const loadComponent = (importFn, name) =>
  dynamic(
    () =>
      importFn().then((mod) => {
        console.log(`Loaded ${name}:`, mod);
        // Handle ES modules with default export or direct export
...
export default function EditProfileForm({ initialData, onSave }) {
  ...
  const [referrerName, setReferrerName] = useState("");
  const [showSuccessModal, setShowSuccessModal] = useState(false);
  const [cityOptions, setCityOptions] = useState(initialData.location ? [{ value: initialData.location, label: initialData.location }] : []);
  const [languageOptions, setLanguageOptions] = useState([]);
  const [selectedExpertise, setSelectedExpertise] = useState(null);
  const [apiError, setApiError] = useState("");
  ...
  // Load CSS with fallback to CDN if local files are missing
  useEffect(() => {
    ...
    const cssFiles = [
      {
        local: "/css/react-phone-input.css",
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        setCityOptions(updatedCities.length > 0 ? updatedCities : [{ value: initialData.location || "Unknown", label: initialData.location || "Unknown" }]);
      } catch (error) {
        console.error("Error fetching cities:", error);
        setApiError(`Failed to load city options: ${error.message}`);
        setCityOptions([{ value: initialData.location || "Unknown", label: initialData.location || "Unknown" }]);
      }
    };
    ...
    const fetchLanguages = async () => {
      try {
        ...
        console.error("Error fetching languages:", error);
        setApiError(`Failed to load language options: ${error.message}`);
        setLanguageOptions([{ value: "English", label: "English" }]); // Fallback
      }
    };
  ...
  const handleCropConfirm = useCallback(async () => {
    try {
      const croppedImage = await import("../complete-profile/getCroppedImg").then(mod => mod.default(imagePreview, croppedAreaPixels));
      const blob = await (await fetch(croppedImage)).blob();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Error saving profile:", error);
      setErrors((prev) => ({ ...prev, submit: `Failed to save profile: ${error.message}` }));
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-2 md:p-4 bg-[#F4D35E]">
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-2xl p-6 md:p-10">
        ...
        {showCropModal && imagePreview && typeof imagePreview === "string" && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl p-6 max-w-lg w-full">
              <h3 className="text-lg font-semibold text-[var(--primary)] mb-4">Crop Your Photo</h3>
              <div className="relative w-full h-64">
                      ...
                      if (inputValue) {
                        fetch(`/api/cities?search=${encodeURIComponent(inputValue)}`)
                          ...
                          .then(data => {
                            ...
                            setCityOptions(updatedCities.length > 0 ? updatedCities : [{ value: formData.location || "Unknown", label: formData.location || "Unknown" }]);
                          })
                          .catch(err => {
                            console.error("Error fetching cities:", err);
